...
%@adjspikespsth/plus Overloaded plus function for adjspikes objects.
...
if(~isa(p,classname))
    ...
    elseif(isempty(q))
        ...
        r.data.psth = concatenate(p.data.psth , q.data.psth);
        r.data.plotVector = concatenate(p.data.plotVector , q.data.plotVector);
