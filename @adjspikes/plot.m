...
%@adjspikes/plot Plot function for adjspikes object.

Args = struct('Color','b','FMarkersOff',0,'RefreshMarkers',0,'Stacked',0,'Burst',0, ...
    'Interleaved',0,'GroupPlots',1,'GroupPlotIndex',1,'Fast',0, ...
    'Highlight',{''},'ReturnVars',{''});

Args.flags = {'HideTitle','showChunk','FMarkersOff','RefreshMarkers', ...
    'Stacked','Burst','Interleaved','Fast'};
...
%         end
%         [mv,mi]=max2(NN);
%         NN=NN./mv; NN=[NN NN(:,1)]; th=[th th(1)];
%         for e=1:num_tf*num_sf
%             subplot(num_tf,num_sf,e)
%             polar(0,1,'--k'); hold on
%             r = polar(th,NN(e,:),'-');
%             get(r); set(r,'LineWidth',2); set(r,'Color',colors(n,:)); 
% %             if e==mi
% %                 polar(th,NN(e,:),['-' Args.Color])
% %             end
%             if e <= num_sf
%                 if e==1
%                     title(['SF: ',num2str(sf(e)) ' cyc/deg'],'FontSize',14)
%                 else
%                     title([num2str(round(sf(e)*100)/100) ' cyc/deg'],'FontSize',14)
%                 end
%             end
%             if e == ((tfi*num_sf)-num_sf)+1
%                 if e==1
%                     ylabel(['TF: ',num2str(round(tf(tfi)*10)/10) ' cyc/deg'],'FontSize',14)
%                 else
%                     ylabel([num2str(round(tf(tfi)*10)/10) ' deg/sec'],'FontSize',14)
%                 end
%                 tfi=tfi+1;
%             end
%         end
...
%     end
    ...
    ylim([-1 rcols+2])
    set(gca,'FontSize',14)
    ylabel('Movie Repetitions','FontSize',18)
    xlabel('Time (seconds)','FontSize',18)
    % set(gca,'YTick',1:125:3000)
    % set(gca,'YTickLabel',stimInfo.data.iniInfo.orientation_angles)
    % ylabel('Drifting Grating Orientation Angle', 'FontSize',18)
