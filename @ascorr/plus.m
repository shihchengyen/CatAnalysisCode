...
if(~isa(p,classname))
    ...
    elseif(isempty(q))
		...
		r.data.ascorr = concatenate(p.data.ascorr,q.data.ascorr,'Columnwise');
        r.data.contcoefs = [p.data.contcoefs; q.data.contcoefs];
		r.data.vang = concatenate(p.data.vang,q.data.vang,'Columnwise');
        r.data.contvang = [p.data.contvang; q.data.contvang];
